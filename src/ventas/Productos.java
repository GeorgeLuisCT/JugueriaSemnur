...
//        FadeEffect.fadeOut(this, 50, 0.1f);
    ...
    private void SeleccionarPorductoVender() {
        ...
        if (this.tabla.getRowCount() < 1) {
            ...
            if (this.tabla.getSelectedRowCount() < 1) {
                ...
                if (this.txtCantidad.getText().equals("")) {
                    ...
                    int indiceFilaSeleccionada = this.tabla.getSelectedRow();
                    String codigo = this.tabla.getValueAt(indiceFilaSeleccionada, 0).toString();
                    String nombre = this.tabla.getValueAt(indiceFilaSeleccionada, 1).toString();
                    float precio = Float.parseFloat(this.tabla.getValueAt(indiceFilaSeleccionada, 3).toString());
                    String cantidad = txtCantidad.getText();

                    float preciototal = precio * Float.parseFloat(cantidad);

                    DefaultTableModel modeloTabla2 = (DefaultTableModel) Ventas.tablaVentas.getModel();
                    ...
                    if (filaExistente != -1) {
                        // Actualizar el registro existente sumando las cantidades
                        String cantidadExistenteStr = modeloTabla2.getValueAt(filaExistente, 2).toString();
                        int cantidadExistente = Integer.parseInt(cantidadExistenteStr);
                        int nuevaCantidad = Integer.parseInt(cantidad);
                        int cantidadTotal = cantidadExistente + nuevaCantidad;
                        modeloTabla2.setValueAt(cantidadTotal, filaExistente, 2);
                        // Actualizar el precio total
                        float nuevoPrecioTotal = precio * cantidadTotal;
                        modeloTabla2.setValueAt(nuevoPrecioTotal, filaExistente, 4);
                    } else {
                        // Agregar un nuevo registro si no existe
                        modeloTabla2.addRow(new Object[]{codigo, nombre, cantidad, precio, preciototal});
                    }

                    // Actualizar el total
                    float total = 0;
                    for (int i = 0; i < modeloTabla2.getRowCount(); i++) {
                        float precioTotal = Float.parseFloat(modeloTabla2.getValueAt(i, 4).toString());
                        total += precioTotal;
                    }
                    Ventas.lblTotal.setText(String.valueOf(total));
